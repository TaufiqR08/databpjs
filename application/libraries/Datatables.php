  ...
  */
  class Datatables
  {
    ...
    */
    ...
    private function exec_replace($custom_val, $row_data) {
      foreach ($custom_val['replacement'] as $key => $val) {
          // Bersihkan string dari kutipan jika ada
          $sval = preg_replace("/(?<!\w)([\'\"])(.*)\\1(?!\w)/i", '$2', trim($val));

          // Jika format function() dan callable
          if (preg_match('/(\w+::\w+|\w+)\((.*)\)/i', $val, $matches) && is_callable($matches[1])) {
              $func = $matches[1];
              // Pisahkan argument function
              $args = preg_split("/[
                  \s,]*\\\"([^\\\"]+)\\\"[
                  \s,]*|[
                  \s,]*'([^']+)'[
                  \s,]*|[,]+/x", $matches[2], 0, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);

              foreach ($args as $i => $arg) {
                  if (preg_match('/^\$[0-9]+$/', $arg)) {
                      $n = str_replace("$", "", $arg);
                      $args[$i] = isset($row_data[$n]) ? $row_data[$n] : null;
                  }
              }
              $replace_string = call_user_func_array($func, $args);
          }
          // Jika variabel seperti $1, $2
          elseif (preg_match('/^\$[0-9]+$/', $val)) {
              $n = str_replace("$", "", $val);
              $replace_string = isset($row_data[$n]) ? $row_data[$n] : null;
          }
          // Nilai biasa
          else {
              $replace_string = $val;
          ...
          // Gantikan placeholder di content dengan hasil akhir
          $custom_val['content'] = str_ireplace("$" . $key, $replace_string, $custom_val['content']);
      }
      return $custom_val['content'];
  }

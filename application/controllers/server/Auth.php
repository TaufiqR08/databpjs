...
class Auth extends CI_Controller
{
	...
	public function cek_login()
	{
		...
		$u 		= $this->security->xss_clean($this->input->post('identity'));
        $p 		= $this->security->xss_clean(md5($this->input->post('password')));
			
		$q_cek	= $this->db->query("SELECT a.*,c.* FROM m_admin a, m_level c WHERE a.level = c.id_level AND a.username = '".$u."' AND a.password = '".$p."'");
        ...
        if($j_cek == 1) {
				$data = array(
                    ...
                    'admin_nama_lengkap' => $d_cek->nama_lengkap,
					'admin_email' => $d_cek->email,
					'admin_id_kecamatan' => $d_cek->id_kecamatan,
					'admin_id_kelurahan' => $d_cek->id_kelurahan,
					'admin_level' => $d_cek->id_level,
					'admin_nama' => $d_cek->nama_level,
			...
			$url = 'server/auth'; // url untuk redirect
		}
	...
	public function forgot_password()
	{
		...
		else
		{
			...
			if (empty($identity))
			{
				...
				redirect("server/auth/forgot_password", 'refresh');
			}
			...
			if ($forgotten)
			{
				...
				redirect("server/auth/forgot_password", 'refresh'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
			...
			else
			{
				...
				else
				{
					...
					if ($change)
					{
						...
						redirect("server/auth/login", 'refresh');
					}
					else
					{
						$this->session->set_flashdata('message', $this->ion_auth->errors());
						redirect('server/auth/reset_password/' . $code, 'refresh');
					}
				}
			}
		}
